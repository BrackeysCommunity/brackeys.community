...
/**
 * Lifecycle sweep (every 6h, the `lifecycle_sweep` repeatable job) — the
 * counterweight to the two rot vectors the board and /teams discovery are
 * exposed to: recruiting posts nobody closes, and husk teams the
 * required-team quick-create mints. Ran as its own cron service until plan
 * 27 folded it in here; the worker already had every dependency but three
 * shared modules.
 *
...
/**
 * The sweep's `notify()` — the shared write path and the same
 * `side_effects` job the app enqueues, onto this worker's own queue. No
 * dedupe window: the lifecycle stamps are what keep re-runs from
 * double-sending.
 */
async function notify(params: Omit<NotifyParams, "actorId" | "dedupeWithin">): Promise<void> {
  ...
  try {
    ...
    console.warn("[lifecycle_sweep] failed to enqueue side-effects", { id: result.id, err });
  }
}
