...
export function createGridOverlay(): GridOverlay {
  ...
  function update(viewport: Viewport): void {
    ...
    const minorStartX = Math.floor(left / MINOR_INTERVAL) * MINOR_INTERVAL;
    const minorStartY = Math.floor(top / MINOR_INTERVAL) * MINOR_INTERVAL;
    ...
    const majorStartX = Math.floor(left / MAJOR_INTERVAL) * MAJOR_INTERVAL;
    const majorStartY = Math.floor(top / MAJOR_INTERVAL) * MAJOR_INTERVAL;
