...
export function CollaborationDetail() {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-2xl font-bold text-white mb-4">Collaboration not found</h1>
  ...
  const statusColor = statusColorMap[post.statusId] || 'bg-gray-500/20 text-gray-400';
        ...
        >
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-4">
              <span className={`px-3 py-1 rounded-md text-sm font-medium border ${typeColor}`}>
                {post.collaborationType?.name}
              </span>
              <span className={`px-3 py-1 rounded-md text-sm font-medium border ${statusColor}`}>
                {post.collaborationStatus?.name}
              </span>
              <span className="text-sm text-gray-400">{post.hiringStatus?.name}</span>
            </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {post.collaborationProfile.skills && (
                  <div>
                    ...
                    <p className="text-gray-200">{post.collaborationProfile.skills}</p>
                  </div>
                )}
                ...
                {post.collaborationProfile.portfolio && (
                  <div>
                    ...
                    <p className="text-gray-200">{post.collaborationProfile.portfolio}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Field Values */}
          {post.collaborationFieldValues && post.collaborationFieldValues.length > 0 && (
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {post.collaborationFieldValues.map(field => (
                  <div key={field.id}>
                    ...
                    <p className="text-gray-200 whitespace-pre-wrap">{field.value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border-t border-gray-700 pt-8">
            ...
            {post.collaborationResponses && post.collaborationResponses.length > 0 ? (
              <div className="space-y-4">
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-medium text-white">
                        {response.collaborationProfile?.displayName || 'Anonymous'}
                      </h3>
                    ...
                    {response.contactInfo && response.isPublic && (
                      <p className="mt-2 text-sm text-gray-400">Contact: {response.contactInfo}</p>
                    )}
                  </motion.div>
                ))}
              </div>
