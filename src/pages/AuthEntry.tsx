...
export const AuthEntry = () => {
  ...
  useEffect(() => {
    ...
    const hasDiscordAccount = user.externalAccounts.some(acc => acc.provider === 'discord');
    ...
    if (!hasDiscordAccount) {
      setErrorMessage('Discord account not connected. Please try logging in with Discord.');
      setStep('error');
      return;
    }
        ...
        >
          <div className="bg-gray-800 rounded-lg shadow-xl border border-red-900/50 p-8">
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-white text-center">Authentication Error</h2>
  ...
  const getStepIcon = () => {
    if (step === 'complete') return <CheckCircle2 className="h-16 w-16 text-white" />;
