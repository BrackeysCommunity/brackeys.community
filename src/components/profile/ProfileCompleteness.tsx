...
export function buildCompletenessItems(data: {
	...
	return [
		...
		},
		{
			label: "Links",
			status:
				data.githubUrl || data.twitterUrl || data.websiteUrl || data.itchIoUrl
					? "done"
					: "empty",
		},
		{
...
const checkboxStyles: Record<CompletenessStatus, string> = {
	...
	pending:
		"border-brackeys-yellow/50 bg-brackeys-yellow/10 text-brackeys-yellow",
				...
				<div className="p-4 space-y-4">
					<div className="flex gap-1">
						{items.map((item) => (
							<div
								key={item.label}
								className="flex-1 h-2 bg-muted/20 overflow-hidden"
							>
								<motion.div
...
export function ProfileCompletenessMini({
	items,
}: {
	items: CompletenessItem[];
}) {
