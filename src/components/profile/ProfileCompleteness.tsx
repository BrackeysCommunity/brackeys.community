...
export function buildCompletenessItems(data: {
  ...
  const approvedProjects = data.projects.filter((p) => p.type !== "jam" && p.status !== "pending");
  const pendingProjects = data.projects.filter((p) => p.type !== "jam" && p.status === "pending");
  ...
  return [
    ...
    { label: "Bio", status: data.bio?.trim() ? "done" : "empty" },
    {
      label: "Skills",
      status:
        data.skills.length > 0 ? "done" : (data.pendingSkillCount ?? 0) > 0 ? "pending" : "empty",
    },
    {
      label: "Links",
      status:
        data.githubUrl || data.twitterUrl || data.websiteUrl || data.itchIoUrl ? "done" : "empty",
    },
    {
      label: "Projects",
      status:
        approvedProjects.length > 0 ? "done" : pendingProjects.length > 0 ? "pending" : "empty",
    },
          ...
          <div className="grid grid-cols-3 gap-x-4 gap-y-2.5">
            {items.map((item) => (
              <div key={item.label} className="flex items-center gap-2">
                ...
                >
                  {item.status === "done" && <HugeiconsIcon icon={Tick01Icon} size={8} />}
                  {item.status === "pending" && <HugeiconsIcon icon={Clock01Icon} size={7} />}
                </div>
                <span
                  className={cn(
