				...
				<div className="flex items-start justify-between gap-2">
					<div className="flex flex-wrap items-center gap-2 min-w-0">
						<span className="font-mono text-[11px] font-bold text-foreground uppercase tracking-wider truncate">
							{project.pinned && (
								<span className="text-brackeys-yellow mr-1.5">*</span>
							)}
							{project.title}
						</span>
						<ProfileProjectTypeBadge
...
function InlineProjectEditForm({
	...
	const handleTypeChange = (nextType: ManualProfileProjectType) => {
		...
		setType(nextType);
		setSubTypes((current) =>
			current.filter((s) => nextAllowed.includes(s)),
		);
	};
...
export function AddProjectForm({
	...
	const handleTypeChange = (nextType: ManualProfileProjectType) => {
		...
		setType(nextType);
		setSubTypes((current) =>
			current.filter((subType) => nextAllowedSubTypes.includes(subType)),
		);
	};
					...
					{previewUrl ? (
						...
						<span className="flex flex-col items-center gap-0.5">
							<HugeiconsIcon
								...
								className="text-muted-foreground/30"
							/>
							<span className="font-mono text-[7px] text-muted-foreground/25 uppercase">
								Image
							</span>
						</span>
					)}
