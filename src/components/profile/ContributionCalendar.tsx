...
const DELTA: Record<Dir, Pos> = { up: { col: 0, row: -1 }, down: { col: 0, row: 1 }, left: { col: -1, row: 0 }, right: { col: 1, row: 0 } };
function posKey(p: Pos) { return `${p.col},${p.row}`; }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSnake((prev) => {
        ...
        if (ate) {
          setFood((f) => { const n = new Set(f); n.delete(nk); return n; });
          setScore((s) => s + 1);
        }
...
function CalendarGrid({ weeks, totalContributions, maxCount, monthHeaders, playing, onToggleSnake }: {
  ...
  return (
    <div className="space-y-1.5">
          ...
          <Fragment key={label || `day${row}`}>
            ...
            {Array.from({ length: cols }, (_, col) => {
              ...
              if (playing) {
                ...
                return <div key={k} className={cn('aspect-square w-full rounded-[2px]', cellClass)} />;
              }
              ...
              return (
                <div
                  key={day.date}
                  className={cn('aspect-square w-full rounded-[2px] transition-colors', intensityClasses[intensity])}
      ...
      {/* Legend */}
      <div className={cn('flex items-center justify-end gap-1 transition-opacity', playing && 'opacity-0')}>
...
export function ContributionCalendar({ userId, className }: ContributionCalendarProps) {
  ...
  const maxCount = useMemo(
    () => data ? Math.max(1, ...data.weeks.flatMap((w) => w.contributionDays.map((d) => d.contributionCount))) : 1,
    [data],
  );
