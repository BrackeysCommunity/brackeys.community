...
const ROOM_SECRET_KEY =
  import.meta.env.VITE_SPACETIME_ROOM_KEY || 'brackeys-default-key';
...
export const RoomManager = ({ onClose }: RoomManagerProps) => {
  ...
  const [selectedColor, setSelectedColor] = useState(
    currentUser?.color || RAINBOW_PALETTE[0],
  );
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    userColor:
      currentUser?.color ||
      RAINBOW_PALETTE[Math.floor(Math.random() * RAINBOW_PALETTE.length)],
    roomCode: '',
    password: '',
    usePassword: false,
  ...
  const [roomTtl, setRoomTtl] = useState(
    currentRoom && currentRoom.messageTtlSeconds > 0
  ...
  > => {
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error('Room join timeout - please try again')),
        10000,
      );
    });
    ...
    try {
      ...
      if (!currentUser?.name && currentFormData.userName?.trim()) {
        await setDisplayName(
          currentFormData.userName.trim(),
          currentFormData.userColor,
        );
      }
      ...
      if (step === 'choice') {
        ...
        const hashedPassword =
          currentFormData.usePassword && currentFormData.password
            ...
            : '';
        const messageTtl =
          currentFormData.messageMode === 'live'
            ? 0
            : currentFormData.messageTtl;
  ...
  if (currentRoom) {
    const isHost =
      currentUser &&
      currentRoom.hostIdentity.toHexString() ===
            ...
            <div className="mb-6 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <RadioGroup
                    value={roomMessageMode}
                    onChange={setRoomMessageMode}
                  >
                    ...
                    >
                      <div>
                        ...
                        <Input
                          ...
                          value={roomTtl}
                          onChange={(value) =>
                            setRoomTtl(Math.max(1, parseInt(value) || 1))
                          }
                          className="font-mono"
                          min={1}
                          placeholder="30"
