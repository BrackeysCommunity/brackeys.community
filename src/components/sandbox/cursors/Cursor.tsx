...
export const CursorContainer = ({
  ...
  const cursorIconAnimation: AnimationControls | TargetAndTransition | VariantLabels | undefined =
    isCurrentUser
      ? {
          scale: [cursorState === 'interactive' ? 1.2 : 1, cursorState === 'typing' ? 0.8 : 1],
          padding: [
  ...
  return (
    <div
      className={cn(
        'absolute z-10',
        isCurrentUser && isTyping ? 'pointer-events-auto' : 'pointer-events-none'
      )}
      style={{
        left: `${position.x}%`,
        top: `${position.y}%`,
    ...
    >
      <div className="relative">
        <motion.div animate={cursorIconAnimation} transition={{ duration: 0.2 }}>
        ...
        {typingState && (
          <TypingBubble
            ...
            onSendMessage={
              onSendMessage ? text => onSendMessage(text, position.x, position.y) : undefined
            }
          />
        )}
      </div>
