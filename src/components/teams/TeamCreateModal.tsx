 ...
 * page.
 *
 * A drawer on every width until §3.3, which is what made START A TEAM read
 * as a different kind of surface from the collab wizard next to it.
 */
export function TeamCreateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <ResponsiveModal
      ...
      onClose={onClose}
      title="Start a team"
      description="Name your team, add a one-liner, and say whether you're taking members."
      dividedHeader
      footer={
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto p-5">
        <Field label="TEAM IMAGE" hint="optional" error={fileError || null}>
          <div className="flex items-center gap-3">
            <UserAvatar avatarUrl={avatar?.previewUrl ?? null} username={name || "?"} size={56} />
