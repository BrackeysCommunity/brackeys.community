...
export const SnakeController = ({
  ...
  }, [onDirectionChange, onReset, onTogglePause, currentDirection, directionDebounceTime]);

  const isDisabled = (direction: Direction): boolean => {
    if (direction === Direction.UP && currentDirection === Direction.DOWN) return true;
    if (direction === Direction.DOWN && currentDirection === Direction.UP) return true;
    if (direction === Direction.LEFT && currentDirection === Direction.RIGHT) return true;
    if (direction === Direction.RIGHT && currentDirection === Direction.LEFT) return true;
    return false;
  };
