...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/10 text-xs/relaxed duration-100 data-closed:animate-out data-closed:fade-out-0 data-ending-style:opacity-0 data-open:animate-in data-open:fade-in-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className,
      )}
      {...props}
    />
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        className={cn(
          ...
          // Same floor as the drawer: a side panel keeps a share of a wide
          // viewport rather than a fixed-rem sliver.
          "data-[side=left]:lg:min-w-[min(28vw,60rem)] data-[side=right]:lg:min-w-[min(28vw,60rem)]",
          className,
        )}
        {...props}
      >
