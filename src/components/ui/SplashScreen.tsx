...
export const SplashScreen = ({ onComplete }: SplashScreenProps) => {
  ...
  const [gradientAnimationComplete, setGradientAnimationComplete] =
    useState(false);
        ...
        <div className="fixed size-full inset-0 flex items-center justify-center">
          {startInitialAnimation && (
            <motion.img
              ...
              transition={{
                duration: !gradientAnimationComplete ? 0.8 : 1.8,
                opacity: { duration: !gradientAnimationComplete ? 0.8 : 1.8 },
                filter: {
                  duration: !gradientAnimationComplete ? 0.8 : 1.8,
                  times: !gradientAnimationComplete ? [0, 0.7, 1] : [0, 0.3, 1],
                  ease: 'easeInOut',
                },
              }}
              onAnimationComplete={() => {
          ...
          <div className="absolute size-full inset-0 flex items-center justify-center">
            {shadowAnimationComplete && !gradientAnimationComplete && (
              <motion.div
                ...
                initial={{
                  backgroundImage:
                    'linear-gradient(to bottom, transparent, transparent, var(--color-brackeys-yellow), var(--color-brackeys-fuscia), var(--color-brackeys-purple), transparent, transparent)',
