...
function ButtonGroupText({ className, render, ...props }: useRender.ComponentProps<"div">) {
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex items-center gap-2 rounded-none border border-[var(--emboss-shadow)] bg-muted px-2.5 text-xs font-medium [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
          className,
        ),
      },
      props,
...
function ButtonGroupSeparator({
  ...
  return (
    <Separator
      ...
      className={cn(
        "relative z-20 self-stretch bg-[var(--emboss-shadow)] data-horizontal:mx-px data-horizontal:w-auto data-vertical:my-px data-vertical:h-auto",
        className,
      )}
      {...props}
    />
