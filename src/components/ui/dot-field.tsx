...
interface ThemedDotFieldProps extends Omit<
  DotFieldProps,
  "gradientFrom" | "gradientTo" | "glowColor"
> {
  /** CSS custom property (with or without `--`) for the gradient start. */
  gradientFromVar?: string;
  /** CSS custom property (with or without `--`) for the gradient end. */
  gradientToVar?: string;
  /** CSS custom property (with or without `--`) for the cursor glow. */
  glowColorVar?: string;
}

/**
 * A {@link DotField} whose colors come from CSS custom properties on
 * `document.documentElement` and re-resolve when the theme changes
 * (i.e. when the `data-theme` or `class` attribute on `<html>` mutates).
 *
 * Defaults resolve to the project's brand tokens: `--primary`, `--chart-2`,
 * and `--background`.
 */
export const ThemedDotField = ({
  gradientFromVar = "primary",
  gradientToVar = "chart-2",
  glowColorVar = "background",
  ...rest
}: ThemedDotFieldProps) => {
  ...
  useEffect(() => {
    const read = () =>
      setColors({
        gradientFrom: getStyle(gradientFromVar),
        gradientTo: getStyle(gradientToVar),
        glowColor: getStyle(glowColorVar),
      });
  ...
  }, [gradientFromVar, gradientToVar, glowColorVar]);
