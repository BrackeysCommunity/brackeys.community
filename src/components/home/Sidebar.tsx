...
export function Sidebar() {
  ...
  const [isDeadlinePassed] = useState(() => Date.now() >= JAM_DEADLINE.getTime());
  ...
  return (
    <div className="flex h-full items-center justify-center p-6">
      <div
        className="pointer-events-auto h-full max-h-[min(800px,calc(100vh-120px))] w-full bg-muted/60"
        style={{ clipPath: notchClip, padding: "2px" }}
      >
        <div
          className="relative flex h-full w-full flex-col bg-background/90 backdrop-blur-md"
          style={{ clipPath: notchClipInner }}
        >
          {/* Corner decorators */}
          <span className="pointer-events-none absolute top-0 left-0 h-2 w-2 border-t border-l border-brackeys-yellow/50" />
          <span className="pointer-events-none absolute right-0 bottom-0 h-2 w-2 border-r border-b border-brackeys-yellow/50" />
          {/* Top-right notch accent */}
          <svg
            aria-hidden="true"
            className="pointer-events-none absolute top-0 right-0 text-brackeys-yellow/40"
            width={NOTCH_SIZE + 2}
            height={NOTCH_SIZE + 2}
          ...
          <svg
            aria-hidden="true"
            className="pointer-events-none absolute bottom-0 left-0 text-brackeys-yellow/40"
            width={NOTCH_SIZE + 2}
            height={NOTCH_SIZE + 2}
            viewBox={`0 0 ${NOTCH_SIZE + 2} ${NOTCH_SIZE + 2}`}
            fill="none"
          ...
          <div className="grid grid-cols-2 divide-x divide-muted/60 border-b border-muted/60">
            <div className="flex flex-col items-center gap-0.5 px-2 py-3">
              ...
              <span
                className={cn(
                  "font-mono text-xl font-bold tracking-tight text-brackeys-yellow tabular-nums transition-opacity duration-300",
              ...
              <>
                <div className="flex h-full items-center justify-center p-5">
                  <img
                    ...
                    className="m-auto w-full max-w-3xl object-cover object-top grayscale transition-all duration-500 hover:grayscale-0"
                    src="https://img.itch.zone/aW1nLzI1NTk5ODA3LnBuZw==/original/vYJgdy.png"
                  />
                </div>
                <div
                  className="animate-scanlines pointer-events-none absolute inset-0 opacity-20"
                  style={{
                    background:
          ...
          {/* Footer */}
          <div className="flex gap-8 border-t border-muted/60 bg-card/30 px-6 py-4">
            <MagneticFooterLink
