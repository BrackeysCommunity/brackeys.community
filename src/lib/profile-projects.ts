export const PROFILE_PROJECT_TYPES = [
	"jam",
	"game",
	"audio",
	"tool",
...
export const MANUAL_PROFILE_PROJECT_TYPES = [
	"game",
	"audio",
	"tool",
	"app",
...
export type ManualProfileProjectType =
	(typeof MANUAL_PROFILE_PROJECT_TYPES)[number];
export type ProfileProjectSource =
	(typeof PROFILE_PROJECT_SOURCE_TYPES)[number];
...
export type AudioProjectSubType =
	(typeof PROFILE_PROJECT_SUBTYPE_OPTIONS.audio)[number];
export type AppProjectSubType =
	(typeof PROFILE_PROJECT_SUBTYPE_OPTIONS.app)[number];
...
export const PROFILE_PROJECT_SUBTYPE_LABELS: Record<
...
export function sanitizeProfileProjectSubTypes(
	...
	for (const subType of subTypes) {
		if (
			!allowedSet.has(subType as ProfileProjectSubType) ||
			seen.has(subType)
		) {
			continue;
		}
